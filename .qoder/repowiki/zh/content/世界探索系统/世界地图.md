...
- 地图渲染引擎、坐标系统与缩放平移交互
- 地图标记系统（位置图标、路径显示、导航提示）
- 地图与游戏状态同步（玩家位置更新、解锁区域显示、探索进度追踪）
- 资源组织与优化（场景图片、WebP格式、响应式适配）
- 自定义开发指南（新地图添加、标记配置、事件绑定）
- 性能优化策略（懒加载、缓存机制、内存管理）

## 项目结构
世界地图由一个主页面与若干模块组成，核心入口为 world_map.html；地图数据与配置集中于 game-config.js；通用工具与存储逻辑位于 game-helpers.js、storage-service.js、idb-storage.js；地点运行与流程控制由 location-runner.js 负责；场景资源以 WebP 形式按地点与昼夜分类存放于 img/location/scene_webp 下。
...
A --> G["img/location/scene_webp/<br/>场景资源(按地点/昼夜)"]
```
...
- 地图标记系统
  - 标记类型包括：当前位置、可进入地点、已解锁/未解锁区域、路径连线、导航箭头等。
  - 标记层级与可见性随缩放级别动态调整，避免重叠与遮挡。
- 状态同步
  - 玩家位置、区域解锁、探索进度等持久化到 IndexedDB，并在地图打开时拉取最新状态进行渲染。
- 资源组织
...
## 架构总览
下图展示世界地图系统的整体架构与关键数据流：页面初始化后从配置加载地图元数据，根据当前地点与昼夜选择场景资源；读取存储中的玩家位置与解锁状态；在渲染循环中计算视口并绘制地图与标记；用户交互触发状态变更并持久化。
...
FetchState --> RenderMap["渲染地图与标记"]
RenderMap --> UserAction{"用户交互?"}
...
## 结论
世界地图系统以模块化架构为基础，结合高效的渲染与交互机制，实现了可扩展、高性能的地图体验。通过清晰的资源组织与完善的同步策略，系统在功能与性能之间取得良好平衡。后续可在标记丰富度、路径规划与多人协作方面持续演进。
